...
def test_generate_returns_response():
    fake_requests = FakeRequests(
        FakeResponse({"response": "Generated answer"})
    )
...
def test_rewrite_query():
    ...
    result = llm.rewrite_query(
        "What did they find about the model's performance?"
    )
...
def test_generate_answer():
    ...
    chunk = TextChunk(
        chunk_id=0,
        page=5,
        text="The model achieved strong performance on the benchmark.",
    )
